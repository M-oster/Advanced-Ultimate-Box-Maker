...
PartMargin = 0.1;
// - Draw PCB
PCBDraw = 0; //[0:No, 1:Yes]
// - Text on PCB
TextOnPCB="DPH5005-Mod.";
...
aAddHoles = [[1,54,103,3],[1,54,128,3],[1,1,4,3],[1,22.5,61,3],[1,16,67.5,3],[1,2,103,3]];
//           L.Bat.Holder R.Bat.Holder  L.BuckConv. R.BuckConv. L.BT-Mod.     R.BT-Mod 
// - Font general (Windows-Fonts)
sTextFont = "Calibri"; // [Arial, Arial Black, Bahnschrift condensed, Calibri, Courier]

/* [Case Feet] */
// - Kind of Case Feet
CaseFeet = 1; // [0:No Feet, 1:OnlyHoles, 2:Rubber Feet, 3:Folding Feet, 4:Folding Feet small]
...
TabThick = 2.5;
// use Inserts
//tab_inserts = 0; //[0:Screws, 1:Snap-tabs EXPERIMENTAL, 2:Insert_M2x4, 3:Insert_M3x5, 4:Insert_M3x7, 5:Insert_M4x5, 6:Insert_M4x6]
...
aFP_Holes = [[1,12,19,6.8,5.9,1],[1,32,19,6.8,5.9,1],[1,23,37,7.6,7.6,0],[1,3.0,57.3,2.0,2.0,0],[1,39.0,57.3,2.0,2.0,0]];
//               PowerJack +         PowerJack -          charge-LED         L.Hole Bat-Displ.       R.Hole Bat-Displ.

// - Square Holes - [On/Off,xPos,yPos,Width,Hight,Filet],[...]
aFP_SHoles = [[1,50,20,74.7,39.2,0]];
//                PowerSupply-Mod.
...
aFP_RTexts = [[0,96,30,2,10,180,0,"1 . 2 . 3 . 4 . 5 . 6"]];
// - Text  [On/Off,xPos,yPos,Fontsize,"Text"],[...]
aFP_STexts = [[1,44,65,6,"Powersupply  0-50V/5A"],[1,5,39,4.0,"Charge"],[0,10,45,4.0,"USB"]];

/* [Backplate Holes] */
// - Square Holes - [On/Off,xPos,yPos,Width,Hight,Filet],[...]
aBP_SHoles = [[1,103,56,19.2,12.9,0],[1,15.0,63.8,11.0,6.2,0],[1,36,47,11.0,21.5,0]];
//                    Power-switch           slider-switch       Automotive Fuse
// References between sliderswitch and its holes
SliderSw_LHoleX=aBP_SHoles[1][1]-6.7;
SliderSw_RHoleX=aBP_SHoles[1][1]+12.1;
SliderSw_HoleY= aBP_SHoles[1][2]+0.2;

// - Square Holes with Recess-Margin -[ OnOff, Sx, Sy, Sl, Sw, Recess, XMargin, YMargin],[...]
aBP_SHolesRecess = [[1,66,65,7.0,3.0,1.8,2.7,2.0]];
//                               USB-conn.
...
// predefined dimensions for the relay-holder
RelayHolder_posX=39.5;
RelayHolder_posY=9.3;
RelayHolder_HoleDist=28.5;
// - Round Holes - [On/Off,xPos,yPos,Diam1,Diam2,Symmetric],[...]
aBP_Holes = [[1,112,42,7.85,7.3,0],[1,118,26,12.0,12.0,0],[1,SliderSw_LHoleX,SliderSw_HoleY,2.0,2.0,0],[1,SliderSw_RHoleX,SliderSw_HoleY,2.0,2.0,0],[1,USBcon_LHoleX,USBcon_HoleY,3.0,3.0,0],[1,USBcon_RHoleX,USBcon_HoleY,3.0,3.0,0],[1,RelayHolder_posX,RelayHolder_posY,3.0,3.0,0],[1,RelayHolder_posX,RelayHolder_posY+RelayHolder_HoleDist,3.0,3.0,0]];
//              Powerjack                Fuse                L.hole SliderSw.    R.hole SliderSw.       L.Hole BT/USB-Sw.    R.Hole BT/USB-Sw.    B.Hole Relay-Holder    T.Hole Relay-Holder

/* [Backplate Text] */
//Text [On/off,xPos,yPos,Fontsize,"Text"],[...]
aBP_STexts = [[1, 10, 60, 5, "USB"],[1, 31, 60, 5, "BT"],[1, 103, 44, 5, "19V"],[1, 108, 27, 5, "5A"],[1,95,60,5,"AC"],[1,129,60,5,"Bat"],[1, 70, 58, 5, "USB"]];
// Round Text [On/off,xPos,yPos,Fontsize,Diameter,Arc,Start,'Text']
aBP_RTexts = [[0,0,0,0,0,0," "]];

/* [FAN guard Settings] */
// - Fan Position Backplane - [On/Off,xPos,yPos],[...]
BP_fan_position = [1,51,18];
// Cover size. E.g. 80 for a 80mm Fan
fan_size_in_mm = 40;//[25:25mm fan, 30:30mm fan, 40:40mm fan, 50:50mm fan, 60:60mm fan, 70:70mm fan, 80:80mm fan, 92:92mm fan, 120:120mm fan, 140:140mm fan]
...
/*  Feet module
   ...
   Added some code for inserts (M.O.)
*/
...
*/
module BottomShell() {
   if(Debug) echo("mod. BottomShell:",PCBFeet=PCBFeet, FootHole=FootHole, pcb_spacer_offset=pcb_spacer_offset, tubings_PCBFeet=tubings[PCBFeet-pcb_spacer_offset][3]);
...
module ShowElectrParts() {
   ...
   if (FPanL) {
      ShowJack(Jx=aFP_Holes[0][1], Jy=aFP_Holes[0][2], color="black");           // aFP_Holes[0]
      ShowJack(Jx=aFP_Holes[1][1], Jy=aFP_Holes[1][2], color="red");             // aFP_Holes[1]
   ...
   if (BPanL) {
      ...
      ShowSwitch(Sx=aBP_SHoles[0][1], Sy=aBP_SHoles[0][2], type=2, color="grey"); // aBP_SHoles[0]
      
      ShowPowerJack(PwrX=aBP_Holes[0][1], PwrY=aBP_Holes[0][2]);                 // aBP_Holes[0]
   
      ShowFuseHolder(posX=aBP_Holes[1][1], posY=aBP_Holes[1][2]);                // aBP_Holes[1]
   
      ShowSlideSwitch(Cx=aBP_SHoles[1][1], Cy=aBP_SHoles[1][2]);                // aBP_SHoles[1] //ß
      
      ShowUSBHolder(Hx=aBP_SHolesRecess[0][1], Hy=aBP_SHolesRecess[0][2], Hz=0);
      ShowUSBModule(Ux=aBP_SHolesRecess[0][1], Uy=aBP_SHolesRecess[0][2], Uz=64);
      
      ShowRelayHolder(Rx=RelayHolder_posX, Ry=RelayHolder_posY);
      
      ShowAutomFuseHolder(posX=aBP_SHoles[2][1], posY=aBP_SHoles[2][2]);    //aBP_SHoles[2]
      
   }  // endif
   
// electr.parts inside the box with absolute position-values
   if (BShell) {
      ShowBatHolders(Bx=106.5, By=52); 
      ShowBatteries(Batx=115.5, Baty=52, type=5);
      ShowBatHolderPillars(Px=103, Py=54);   // Start-pos of the two pillars
   
      ShowBuckConverter(Bx=0, By=4, Bz=5);
   
      ShowBTModule(Bx=0, By=65, Bz=5);
